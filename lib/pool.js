...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var updateScore = generateScoreUpdate(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate);
    ...
    if (blockCandidate){
        ...
        if(config.payoutScheme == PAYOUT_SCHEME_WINDOW){
    ...
    redisClient.multi(redisCommands).exec(function(err, replies){
        ...
        if (config.payoutScheme == PAYOUT_SCHEME_SLUSH) {
...
function generateScoreUpdate(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'config.payoutscheme ' + config.payoutScheme);

    var scheme = config.payoutScheme ? config.payoutScheme : '';
    ...
    log('info', logSystem, 'chosen scheme ' + config.payoutScheme);
    ...
    switch(scheme){
        ...
        case PAYOUT_SCHEME_WINDOW:
            ...
            log('info', logSystem, 'using payout scheme window');

            var weight = ((new Date().getTime() - miner.startTime) / 3600000);

            log('info', logSystem, 'weight is ' + weight + ', start time was ' + miner.startTime);
